// SERVER ACTUALIZADO PARA 10 BYTES DE HEADER
...
long long readLongLong(int sock, int n) {
    string s = readN(sock, n);
    if (s.size() != n || s.find_first_not_of("0123456789") != string::npos) {
        cerr << "Error: datos no numericos en readLongLong(" << n << "): [" << s << "]" << endl;
        throw invalid_argument("readLongLong: formato invalido");
    }
    return stoll(s);
}
...
void broadcast(const string &mensaje, int fromSock) {
    ...
    long long total = 1 + 10 + 10 + sender.size() + mensaje.size();
    stringstream ss;
    ss << setw(10) << setfill('0') << total;
    ss << 'b';
    ss << setw(10) << setfill('0') << mensaje.size();
    ss << setw(10) << setfill('0') << sender.size();
    ss << sender;
    ss << mensaje;
    string data = ss.str();
...
void enviarMensaje(const string &mensaje, const string &destino, int fromSock) {
    ...
    string sender = clientes[fromSock];
    long long total = 1 + 10 + mensaje.size() + 10 + sender.size();
    stringstream ss;
    ss << setw(10) << setfill('0') << total;
    ss << 'M';
    ss << setw(10) << setfill('0') << mensaje.size();
...
void forwardFile(const string &dest, const string &filename, const vector<char> &content, int fromSock) {
    ...
    long long total = 1 + 10 + dest.size() + 10 + filename.size() + 18 + contentSize;
    stringstream ss;
    ss << setw(10) << setfill('0') << total;
    ss << 'f';
    ss << setw(10) << setfill('0') << dest.size() << dest;
    ss << setw(10) << setfill('0') << filename.size() << filename;
...
void readSocketThread(int cli) {
    while (true) {
        string header = readN(cli, 10);
        if (header.size() < 10) break;
        long long totalLen = stoll(header);
        ...
        if (type == 'n') {
            ...
            int lenMsg = readLongLong(cli, 10);
            string mensaje = readN(cli, lenMsg);
            int lenDest = readLongLong(cli, 10);
            ...
            int lenMsg = readLongLong(cli, 10);
            int lenSender = readLongLong(cli, 10);
            ...
            int lenName = readLongLong(cli, 10);
            string filename = readN(cli, lenName);
            int lenContent = readLongLong(cli, 18);
