...
string formatFileMessage(const string &filename, const string &destino) {
    ...
    long long total = 1 + 10 + destino.size() + 10 + filename.size() + 18 + contentSize;
    ...
    ss << setw(10) << setfill('0') << destino.size();
    ss << destino;
    ss << setw(10) << setfill('0') << filename.size();
    ss << filename;
...
void readSocketThread(int cli) {
    while (true) {
        ...
        if (type == 'M') {
            int lenMsg = stoi(readN(cli, 10));
            string mensaje = readN(cli, lenMsg);
            int lenSender = stoi(readN(cli, 10));
            ...
            int lenDest = stoi(readN(cli, 10));
            string dest = readN(cli, lenDest);
            int lenName = stoi(readN(cli, 10));
            ...
            cout << "\nServidor indico cierre de conexion." << endl;
            break;
        }
    }
...
int main() {
    ...
    {
        stringstream ss;
        ss << setw(10) << setfill('0') << usuario.size();
        ss << 'n';
        ss << usuario;
